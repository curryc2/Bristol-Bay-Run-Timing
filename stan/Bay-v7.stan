...
parameters {
  ...
  // Residual SD in regression relationships
  real <lower=0> RPI_sigma;
  real <lower=0> TT_sigma;
  
  // Version 7 treats RPI and TT as parameters, informed by regression relationship
...
model {
  ...
  //
  for (y in 1:Nyear) {
    RPI[y] ~ normal(RPI_int + dot_product(RPI_slp, covarRPI[,y]), RPI_sigma);
    TT[y] ~ normal(TT_int + dot_product(TT_slp, covarTT[,y]), TT_sigma);
  }// next year
